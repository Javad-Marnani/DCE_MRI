...
'''Extract the files from the RAR archive if you haven't done so already.'''
#Archive("DATA/Label0.rar").extractall("DATA/Balanced_Data/Duke-Breast-Cancer-MRI")
#Archive("DATA/Label1.rar").extractall("DATA/Balanced_Data/Duke-Breast-Cancer-MRI")
#Archive("DATA/Label2.rar").extractall("DATA/Balanced_Data/Duke-Breast-Cancer-MRI")
#Archive("DATA/Label3.rar").extractall("DATA/Balanced_Data/Duke-Breast-Cancer-MRI")
...
def save_dcm_slice(dcm_fname, label, vol_idx, seq_type):
    ...
    target_png_dir = 'DATA/target_{}'.format(seq_type)
...
# Number of all cancer-containing slices related to 200 selected patients
...
# Dictionary containing the count of cancer-containing slices for each patient
count_patient_slices(seq_type='post_3', label='pos')
...
process_paths(seq_type='post_2', label='pos', show=50)
...
# Sort the slices and obtain the sorted list
sort_slices(seq_type='post_1', label='pos')
...
# Sort the paths and obtain the sorted list
sort_paths(seq_type='post_3', label='pos')
...
def plot_cropped_images(boxes_path,seq_type='pre', label='pos', show=False):
    ...
    for i in range (len(sorted_paths)):
        ...
        if show and (i+1) < int(show):
            plt.show()
        else:
            break

boxes_path ='DATA/Annotation_Boxes .csv'
plot_cropped_images(boxes_path,seq_type='pre', label='pos', show=40)
...
def crop_and_save_images(boxes_path,seq_type='pre',crop='original', label='pos', show=False):
    ...
    'Resized Images'/+seq_type+'_'+crop directory. Optionally, the function can display the cropped images
    ...
    for i in range (len(sorted_paths)):
        ...
        if type(crop)==int:
            crop=str(crop)
        output_directory='Resized Images/'+seq_type+'_'+crop
...
for seq_type in tqdm(['pre','post_1','post_2','post_3']):
    for crop in  tqdm(['original',32,64]):
        crop_and_save_images(boxes_path,seq_type, crop,label='pos', show=20)
...
def extract_pixels(seq_type='pre',crop='original'):
    ...
    for i in range(n):
        ...
        path = f"Resized Images/{seq_type}_{crop}/img_{seq_type}_{str(i).zfill(4)}.png"
...
# Plot the first 100 images from the 'pixel' matrix
plot_images(seq_type='pre',crop='original', num_images=100)
...
# Perform threshold segmentation on the 'pixel' array with a threshold of 30 and display intermediate results for up to 60 images
mask = threshold_segmentation(seq_type='post_1',crop=64, threshold=30, show=50)
...
for seq_type in tqdm(['pre','post_1','post_2','post_3']):
    for crop in  tqdm(['original',32,64]):
        ...
        filename = f"Extracted Features/{seq_type.capitalize()}_{str(crop).capitalize()}.csv"
        D.to_csv(filename, header=True)

# Load the CSV file 'Pre_Original.csv' into a pandas DataFrame
D = pd.read_csv('Extracted Features/Pre_Original.csv')
