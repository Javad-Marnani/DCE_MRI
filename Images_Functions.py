...
def plot_cropped_images(boxes_path,seq_type='pre', label='pos', show=False):
    ...
    for i in range (len(sorted_paths)):
        ...
        left = boxes_df.iloc[(patient_num - 1)].iloc[3]
        right = boxes_df.iloc[(patient_num - 1)].iloc[4]
        top = boxes_df.iloc[(patient_num - 1)].iloc[1]
        bottom = boxes_df.iloc[(patient_num - 1)].iloc[2]
        width = right - left
        height = bottom - top
